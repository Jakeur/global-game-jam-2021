...
public class RagdollController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Q))
        {
            ...
            {
                if (bone.gameObject.CompareTag("LeftArm"))
                {
                    Debug.Log("Left arm is find: " + bone.gameObject.name);
                    bone.transform.Rotate(xAngle, yAngle, zAngle, Space.Self);
        ...
        if (Input.GetKeyDown(KeyCode.D))
        {
            ...
            foreach (Rigidbody bone in bones)
            {
                if (bone.gameObject.CompareTag("LeftUpArm"))
                {
                    Debug.Log("Left Up arm is find: " + bone.gameObject.name);
                }
            }
        }
